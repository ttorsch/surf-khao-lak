...
export default function PhotoCarousel({
  ...
  slides: Slide[];
  /** Nom du carrousel pour les lecteurs d'écran */
  label: string;
  intervalMs?: number;
}) {
  ...
  return (
    <div
      ...
      aria-label={label}
      className="relative h-[260px] overflow-hidden rounded-card shadow-card sm:h-96"
    >
      ...
      {/* Au-delà de huit photos, une rangée de pastilles déborde sur deux lignes
          et recouvre l'image : on bascule sur un compteur. */}
      {slides.length > 8 ? (
        <p className="absolute inset-x-0 bottom-4 flex justify-center">
          <span className="rounded-full bg-sand-50/90 px-3 py-1 text-[13px] font-semibold text-navy tabular-nums">
